...
const Hero = () => {
  ...
  const handleSearch = async (departureInput, arrivalInput, dateData, optionsData) => {

    // Check for empty inputs
    if (!departureInput || !arrivalInput || !dateData || !optionsData) {
      toast.error("Please fill in all the required fields...");
      return;
    }
  
    // Check if the user is logged in
    const isAuthenticated = checkUserAuthentication(); // Replace with your actual authentication check
  
    if (!isAuthenticated) {
      toast.error("Please log in before searching for flights...");
    ...
    try {
      const requestData = {
        ...
        startDate: format(dateData[0].startDate, 'yyyy-MM-dd'),
        endDate: format(dateData[0].endDate, 'yyyy-MM-dd'),
        adult: optionsData.adult,
        minor: optionsData.minor,
      };
  
      const response = await axios.post('http://localhost:3000/trip/search', requestData);
  
      console.log('Response:', response.data);
      toast.success("Searching Flights...");
  
      // Redirect to the Explore page upon successful search
      navigate('/explore');
    } catch (error) {
  ...
  const checkUserAuthentication = () => {
    ...
    // Return true if a valid token exists, indicating the user is authenticated
    return !!userToken;
  };
      ...
      <header className="flex flex-col items-center relative w-full h-[529px] px-7 py-4">
        ...
        onFocus={() => arrivalSuggest.setIsOpen(true)}
          ...
          <div className="flex w-full h-full justify-start items-center border-[1px] border-[#CBD4E6]  p-2">
            ...
            <button className="w-full bg-[#605DEC] text-[#FAFAFA] text-lg leading-6 h-[45px] lg:h-[65px] px-5   lg:rounded-r-[4px]"zz
             onClick={() => handleSearch(departureSuggest.input, arrivalSuggest.input, date, options)}>
              Search
            </button>
